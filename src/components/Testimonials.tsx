...
export default function Testimonials() {
  ...
  const renderCard = (review: typeof reviews[0], index: number, prefix: string) => (
    <div
      key={`${prefix}-${index}`}
      className="w-[290px] sm:w-[320px] md:w-[370px] shrink-0 bg-white p-6 md:p-8 rounded-[1.5rem] shadow-[0_4px_20px_rgb(0,0,0,0.01)] border border-foreground/5 flex flex-col justify-between hover:border-primary/20 hover:shadow-[0_10px_30px_rgba(155,48,48,0.03)] hover:-translate-y-1 transition-all duration-300 group"
    >
      <div>
        <div className="flex justify-between items-center mb-4 md:mb-5">
          ...
          {/* Subtle Quote Symbol */}
          <svg className="w-6 h-6 text-primary/10 group-hover:text-primary/20 transition-colors duration-300" fill="currentColor" viewBox="0 0 24 24">
            <path d="M14.017 21v-7.391c0-5.704 3.731-9.57 8.983-10.609l.995 2.151c-2.432.917-3.995 3.638-3.995 5.849h4v10h-9.983zm-14.017 0v-7.391c0-5.704 3.748-9.57 9-10.609l.996 2.151c-2.433.917-3.996 3.638-3.996 5.849h3.983v10h-9.983z" />
          </svg>
        </div>
        <p className="font-serif text-[0.95rem] md:text-[1.05rem] leading-relaxed text-foreground mb-5 md:mb-6 font-light">
          &quot;{review.text}&quot;
        </p>
      </div>
      <div className="flex flex-col border-t border-foreground/10 pt-4 mt-auto">
        <span className="font-bold text-[0.75rem] sm:text-xs tracking-wider text-foreground uppercase mb-0.5">
          {review.name}
        </span>
        <span className="text-primary text-[0.65rem] sm:text-[0.7rem] font-semibold tracking-widest uppercase">
          {review.service}
        </span>
      </div>
    </div>
  ...
  return (
    <section className="w-full bg-background flex justify-center py-16 md:py-20 px-4 md:px-6 overflow-hidden relative">
      ...
      <div className="w-full max-w-[1400px] flex flex-col gap-10 md:gap-14 relative z-10">
        ...
        <motion.div 
          ...
          className="flex flex-col lg:flex-row justify-between items-end gap-5 md:gap-8 px-4"
        >
          <div className="flex-1">
            <div className="uppercase tracking-[0.2em] text-[0.7rem] font-bold mb-3 md:mb-4 text-primary">
              CLIENT LOVE
            </div>
            <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl leading-[1.15] tracking-tight text-foreground">
              What Our Clients Say
            </h2>
          </div>
          <div className="flex-1 lg:text-right text-foreground/60 font-light text-sm md:text-base">
        ...
        >
          ...
          <div className="w-full marquee-container overflow-hidden py-1">
            <div className="flex gap-4 md:gap-6 animate-marquee pause-on-hover w-max">
          ...
          {/* Row 2: Left-to-Right Scrolling (Counter Direction) */}
          <div className="w-full marquee-container overflow-hidden py-1">
            <div className="flex gap-4 md:gap-6 animate-marquee-reverse pause-on-hover w-max">
              {/* Duplicated list of reviews for a seamless loop */}
